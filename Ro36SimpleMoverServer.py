...
class SimpleRobotMoverServer(Node):
    ...
    def _goal_callback(self, goal_request):
        self.get_logger().info('Received goal request with target pose ' + self._pose_as_string(goal_request.pose))
    ...
    def _cancel_callback(self, goal_handle):
        self.get_logger().info('Cancelling move to pose ' + self._pose_as_string(goal_handle.request.pose))
        return CancelResponse.ACCEPT

    def _execute_callback(self, goal_handle):
        self.get_logger().info('Executing move to pose ' + self._pose_as_string(goal_handle.request.pose))
    ...
    def _dist_to_goal(self, goal_handle):
        return float(math.sqrt((goal_handle.request.pose.x - self._last_pose_x)**2 
                                                             + (goal_handle.request.pose.y - self._last_pose_y)**2))
    ...
    def _determine_action_result(self, goal_handle):
        ...
        if goal_handle.is_active and self._dist_to_goal(goal_handle) <= 0:
            self.get_logger().info('Move to pose ' + self._pose_as_string(goal_handle.request.pose) + ' succeeded')
            goal_handle.succeed()
            result.reached = True
        elif goal_handle.is_cancel_requested:
            goal_handle.canceled()
            self.get_logger().info('Move to pose ' + self._pose_as_string(goal_handle.request.pose) + ' was canceled')
        else:
            if goal_handle.is_active:
                goal_handle.abort()
                self.get_logger().info('Move to pose ' + self._pose_as_string(goal_handle.request.pose) + ' was aborted')
        return result


    def _pose_as_string(self, pose):
